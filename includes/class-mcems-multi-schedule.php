...
class MCEMS_Multi_Schedule {
    ...
    public static function save_schedule_times( int $post_id, \WP_Post $post, bool $update ): void {
        ...
        if ( ! empty( $times ) ) {
            ...
            // IMPORTANT:
            // The base plugin already creates one session for every valid
            // date × time combination when session_times[] is posted.
            // Re-generating sibling sessions here causes duplicates and,
            // because save_post fires before the base flow finishes updating
            // all session meta, those duplicates can be created without the
            // correct exam/date data and fall back to capacity 1.
            //
            // Premium therefore limits itself to storing the submitted time
            // array for display/helpers and does NOT create any extra posts.
